...
class BatchNorm1d(nn.Module):
	...
	def __init__(self, num_features):
		...
		# Parameters
		self.weight = Parameter(torch.randn(num_features, dtype=torch.float32))
		self.bias = Parameter(torch.randn(num_features, dtype=torch.float32))
		self.mean = torch.zeros(num_features)
		self.var = torch.ones(num_features)
		# Store the average mean and variance
		self.register_buffer('running_mean', self.mean)
		self.register_buffer('running_var', self.var)
	...
	def forward(self, input):
		...
		self.mean = torch.zeros(input.shape)
		self.var = torch.ones(input.shape)
		mean = input.mean(dim=0)
		var = input.var(dim=0, unbiased=False)
		bn_init = (input - mean) / torch.sqrt(var + self.eps)
		bn = self.weight * bn_init + self.bias
		input = bn
		return input
